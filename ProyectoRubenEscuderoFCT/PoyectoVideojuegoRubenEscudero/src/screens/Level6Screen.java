...
import characters.BossLevel5;
import characters.BossLevel6;
import characters.Enemy;
import characters.Enemy2;
import characters.Enemy3;
import characters.EnemyExtra1;
...
import items.SuperPowerUp;
import tiled.TilemapActor;
import util.DialogBox;

/**
 * Class to play level 6
 * @author ruben
 * @since 22/04/2020
 */
public class Level6Screen extends BaseScreen{
	...
	private boolean xEnemy;
	private float xEnemyPosition;
	...
	public void initialize() {
		...
		background.loadTexture("assets/backgrounds/extra1.png");
		background.setSize(512, 64000);
		...
		//Tiled
		tma = new TilemapActor("assets/tiled/Level5.tmx", mainStage);
	   
	    for(MapObject obj: tma.getTileList("Shield")) {
	        MapProperties props = obj.getProperties();
	        new SuperPowerUp((float)props.get("x"), (float)props.get("y"), mainStage);
	    }
        ...
        boss = new BossLevel6(0, 600, mainStage);
        boss.setLife(8000);
        ...
        try {
			...
			basePoints = Integer.parseInt(properties.getProperty("actualpoints6"));
		...
		music = Gdx.audio.newMusic(Gdx.files.internal("assets/music/Transonic Gravity.mp3"));
	...
	public void update(float dt) {
        ...
        for(BaseActor powerUp: BaseActor.getList(mainStage, SuperPowerUp.class.getName())) {
        ...
        for(BaseActor spaceBullet: BaseActor.getList(mainStage, SpaceBullet2.class.getName())) {
        	...
        	for(BaseActor enemy: BaseActor.getList(mainStage, EnemyExtra1.class.getName())) {
        ...
        if(xEnemy) {
        	xEnemyPosition = xEnemyPosition + 5.00f;
        	if(xEnemyPosition == 50.00f) {
        		xEnemy = false;
        	}
        }
        else {
        	xEnemyPosition = xEnemyPosition - 5.00f;
        	if(xEnemyPosition == 0.00f) {
        		xEnemy = true;
        	}
        }
        ...
        for(BaseActor enemy: BaseActor.getList(mainStage, EnemyExtra1.class.getName())) {
    		...
    		//enemy.setX(xEnemyPosition + enemy.getX());
    	}
        ...
        if(bossDie) {
            ...
            if(points > basePoints) {
            	properties.setProperty("actualpoints6", String.valueOf(points));
            	try {
        ...
        boss.setY(yBossPosition + player.getY() + 499);
        
        if(xBoss) {
        	xBossPosition = xBossPosition + 5.00f;
        	if(xBossPosition == 330.00f) {
        		xBoss = false;
        	}
        }
        else {
	...
	/**
	 * Method to create powerful bullets who attack enemies
	 */
	private void shootPlayerPower() {
		if(!gameOver && !bossDie) {
			long time = System.currentTimeMillis();
			if(lastShoot - time < -100) {
				lastShoot = time;
	        	...
	        	SpaceBullet2 spaceBullet2 = new SpaceBullet2(player.getX() - 10, player.getY(), mainStage);
				spaceBullet2.setRotation(player.getFacingAngle());
	        	spaceBullet2.setMotionAngle(90);
	        	
	        	SpaceBullet2 spaceBullet3 = new SpaceBullet2(player.getX() + 60, player.getY(), mainStage);
				spaceBullet3.setRotation(player.getFacingAngle());
	        	spaceBullet3.setMotionAngle(90);
	        	laser.play(effectVolume);
			}
		}	
	}
	...
	/**
	 * Method to create two small bullets who attack enemies
	 */
	...
	private void newEnemy() {
		long time = System.currentTimeMillis();
		if(lastEnemy - time < -800) {
			lastEnemy = time;
			EnemyExtra1 enemy = new EnemyExtra1(MathUtils.random(50, 400), player.getY() + 650, mainStage);
			enemy.setLife(10);
		}
	}
